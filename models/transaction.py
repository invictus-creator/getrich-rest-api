...
class TransactionModel(db.Model):
    ...
    def update_prices(cls, name, _type):
        if _type == "Income":
            db.session.query(cls).filter(cls.category == name).update({"price": "price=abs(price)"})
        else:
            db.session.query(cls).filter(cls.category == name).update({"price": "price=abs(.price)*-1"})
